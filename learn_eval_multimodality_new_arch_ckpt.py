...
class TrainerNewArch():
    ...
    def save_model(self, model, ep):
        os.makedirs(os.getcwd()+'/model_pytorch/' + 'bc_fixed_multiple_02/'+self.name, exist_ok=True)
        model_name = self.name+'_'+self.get_git_commit_hash()[0:4]+'_ep_'+f'{ep}'+'.pkl'
        torch.save(model.state_dict(), os.getcwd()+'/model_pytorch/' + 'bc_fixed_multiple_02/' + model_name)
        return model_name
...
# ----------------------------------------------------------------------------------
    ...
    models = models_0 + models_1 + models_2
    ...
    for extra_steps in extra_steps_list:
        for model_path in models:
            ...
            persist_points = None
            diff_bc_video = f'diff_bc_video_(diffuser)/birdview/Fixed_Route_3/BC/{model_path.split("/")[1]}_{extra_steps}_extra_steps/'
