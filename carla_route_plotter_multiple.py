...
class CarlaRoutePlotter:
    ...
    def _create_base_image(self) -> None:
        """Create the initial base image from CARLA with 20% opacity."""
        camera = self._create_camera()
        self.world.tick()
        result = camera.get()  # Assuming result is a NumPy array (H, W, 3)

        # Convert to RGBA
        image = Image.fromarray(result).convert("RGBA")

        # Apply 20% opacity (alpha = 51 out of 255)
        r, g, b, _ = image.split()
        alpha = Image.new("L", image.size, int(255 * 0.2))  # 20% of 255 ≈ 51
...
def get_txt_files_with_epoch(path: Union[str, Path], epoch: int) -> List[str]:
    ...
    # pattern = re.compile(rf".*0new_arch_26b7_ep_{epoch}_.+\.txt$")
    pattern = re.compile(rf"^version_750_.*_ep_{epoch}_\d+\.txt$")

    # version_750_0new_arch_26b7_ep_500_0

    for file in path.glob("*.txt"):
        if pattern.match(file.name):
            if 'distance_traveled' in str(file.resolve()):
                continue
            txt_files.append(str(file.resolve()))
...
def plot_direction_histogram_from_csvs(folder_path, output_image_path, min_epoch=20, max_epoch=600):
    ...
    Args:
        folder_path (str): Caminho da pasta onde estão os arquivos.
    ...
    """
    epoch_files = []
    # Regex: extrai o número logo depois de "ep_" seguido de "_" e termina com ".csv"
    pattern = re.compile(r"ep_(\d+)_.*\.csv$")
    ...
    # Ordenar os arquivos por epoch crescente
    epoch_files.sort()
    ...
    if not x_labels:
        print("Nenhum dado encontrado nos arquivos dentro do intervalo de epochs.")
        return
...
if __name__ == '__main__':
    ...
    # for epoch in [20, 30, 60, 70, 110, 150, 220]:
    #     # epoch = 220
    #     caminho = 'diff_bc_video_(diffuser)/birdview/Multiple_dbc_and_d2bc_t_intersec/Diffusion-BC/Diffusion_BC_Multi_Multiple_New_Arch_Full_0_extra_steps/version_750_0/'
    #     lista_de_txts = get_txt_files_with_epoch(caminho, epoch)
    #     classify_last_y_values(lista_de_txts, caminho + f'CARLA_{epoch}.csv')
    ...
    for round in [1]:
        for ep in [40, 60, 100, 180, 220]:
            # caminho = f"diff_bc_video_(diffuser)/birdview/t_interction/BC/BC_Multi_Simple_0{round}_0_extra_steps/BC_Multi_Simple_0{round}/"
            ep_0 = [50, 330, 340, 640, 680]
            ep_1 = [80, 100, 130, 140, 220]
            ep_2 = [30, 80, 130, 200, 380]
            ep_3 = [40, 130, 150, 170, 180]
            ep_4 = [50, 100, 210, 230, 500]
            caminho = f"diff_bc_video_(diffuser)/birdview/Multiple_at_t/BC/BC_t_insersection_multiples_0_extra_steps/BC_t_insersection_multiples/"
            epoch = ep
            ...
            plotter.load_multiple_routes(lista_de_txts, caminho + f'CARLA_bc_{epoch}.png')
