...
def evaluate_agent(ckpt_path):
    ...
    saved_variables = th.load(ckpt_path, map_location='cpu')
    ...
    policy.load_state_dict(saved_variables)
    device = 'cpu'
    policy.to(device)

    routes_completed = []
    fixed_camera = Camera(env.env_world, image_size, image_size, 90, fixed_camera_x, fixed_camera_y, camera_z, -90, 0)
...
if __name__ == '__main__':
    ...
    #     ckpt_idx += 4
    ckpt_file = pathlib.Path('model_pytorch/Diffusion_BC_Multi_Simple_New_Arch/version_750_3/new_arch_2ac8_ep_90.pkl')
