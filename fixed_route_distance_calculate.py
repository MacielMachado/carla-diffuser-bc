    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".csv") and "ep_" in filename:
            ...
            if "route_completed_in_m" in df.columns:
                mean_value = df["route_completed_in_m"].mean()
...
if __name__ == '__main__':
    ...
    compute_relative_completion_means_from_csv(
        "diff_bc_video_(diffuser)/birdview/Fixed_Route_Multiple/BC/bc_fixed_multiple_02_0_extra_steps/bc_fixed_multiple_02"
    )
    compute_relative_completion_means_from_csv(
        "diff_bc_video_(diffuser)/birdview/Fixed_Route_Multiple/Diffusion-BC/dbc_fixed_multiple_02_0_extra_steps/dbc_fixed_multiple_02"
    )
