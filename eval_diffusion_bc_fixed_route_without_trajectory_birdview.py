...
def evaluate_policy(env, model, video_path, device, max_eval_steps=3000, observation_type='birdview',  architecture='diffusion', movie=True):
    ...
    ep_dict['state'] = []
    infractions = 0
    route_completed_in_m_counter = 0
    route_completed_in_m_previous = 0

    while n_step < max_eval_steps or infractions > 0:
        ...
        print(f"n_step: {n_step}; infractions: {infractions}; route_completed_in_m: {info['route_completion']['route_completed_in_m']}; x: {info['gnss'][0]}; y: {info['gnss'][1]}")
        
        infractions = info['collisions_layout'] + info['route_deviation'] + info['wrong_lane'] + info['outside_lane'] + info['terminal_debug']['c_blocked'] + info['terminal_debug']['c_lat_dist']
        
        if route_completed_in_m_previous == info['route_completion']['route_completed_in_m']:
            route_completed_in_m_counter += 1
            if route_completed_in_m_counter > 100:
                infractions = 1
        else:
            route_completed_in_m_previous = info['route_completion']['route_completed_in_m']
            route_completed_in_m_counter = 0
...
if __name__ == '__main__':
    ...
    for model_path in models:
        ...
        for i in range(0, 10):
            ...
            diff_bc_video = 'diff_bc_video_(diffuser)/birdview/town01_fixed_route_no_trajectory_with_actions/'
