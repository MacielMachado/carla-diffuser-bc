...
def handle_obs(obs, observation_type, use_velocity, use_greyscale):
    obs = DataHandler().preprocess_images(obs,
...
def evaluate_policy(env, model, video_path, device, max_eval_steps=3000, observation_type='birdview',  architecture='diffusion', movie=True, use_velocity=False, use_greyscale=True):
    model = model.eval()
    t0 = time.time()
    ...
    ep_dict['state'] = []
    speed = [0, 0]
    previous_actions = torch.tensor([[0, 0]])
    ...
    while n_step < max_eval_steps:
        ...
        previous_actions = actions.unsqueeze(0)
        ...
        obs = handle_obs(obs_clean, observation_type, use_velocity, use_greyscale)
